...
## Allocation that maximizes life-years saved
...
\pagebreak
## Simulated ICU population from CDC Data
```{r}
set.seed(12345)
tot_patients <- 2000
max_life_span <- 100
...
A random allocation of `r num_vents` ventilators would save `r lives_saved(lottery_allocation)` out of `r tot_patients` patients admitted to the ICU. A lottery saves `r  comma(life_years_saved(lottery_allocation))` (`r pct_life_years_saved`%) out of a total of possible `r comma(total_life_years)` life years.
...
Youngest first allocation `r num_vents` ventilators would save `r young_lives` out of `r tot_patients` patients admitted to the ICU. Youngest first saves `r  comma(young_lf)` (`r comma(100*young_lf/total_life_years)`%) out of a total of possible `r comma(total_life_years)` life years.
...
A $P(ICU \space survival)$ triage system of `r num_vents` ventilators would save `r max_icu_lives` out of `r tot_patients` patients admitted to the ICU. Maximizing ICU survival saves `r comma(max_icu_lf)` out of a total of possible `r comma(total_life_years)` (`r round(100*max_icu_lf/total_life_years)`%) life-years.
