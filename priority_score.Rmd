---
title: "Evaluating Ventilator Allocation Strategies During the COVID-19 Pandemic"
...
sickest_first_allocation <- sickest_first(sim_pop, num_vents)
```

```{r}
sickest_first_allocation %>%
  ggplot(aes(x = age, y = SOFA, color = get_vent)) +
  geom_point() + labs(color = "") +
  scale_y_discrete(drop = FALSE)
```
Sickest first allocation of `r num_vents` ventilators would save `r lives_saved(sickest_first_allocation)` out of `r tot_patients` who were in need of mechanical ventilation. Sickest first saves `r  comma(life_years_saved(sickest_first_allocation))` (`r round(100*life_years_saved(sickest_first_allocation)/total_life_years)`%) out of a total of possible `r comma(total_life_years)` life years.
...
## Lottery- age vs. SOFA
```{r}
lottery_plot <- lottery_allocation %>%
  ggplot(aes(x = age, y = SOFA, color = get_vent)) +
  geom_point() + labs(color = "") +
  scale_y_discrete(drop = FALSE)
...
lottery_plot
```
A random allocation of `r num_vents` ventilators would save `r lives_saved(lottery_allocation)` out of `r tot_patients` who were in need of mechanical ventilation. A lottery saves `r  comma(life_years_saved(lottery_allocation))` (`r round(100*life_years_saved(lottery_allocation)/total_life_years)`%) out of a total of possible `r comma(total_life_years)` life years.
...
```{r}
max_icu_plot <- max_icu_surv_allocation %>%
  ggplot(aes(x = age, y = SOFA, color = get_vent)) +
  geom_point() + labs(color = " ") +
  scale_y_discrete(drop = FALSE)
...
max_icu_plot
```
Maximizing lives saved with `r num_vents` ventilators would save `r max_icu_lives` out of `r tot_patients` who were in need of mechanical ventilation. This approach saves `r comma(max_icu_lf)` out of a total of possible `r comma(total_life_years)` (`r round(100*max_icu_lf/total_life_years)`%) life-years.
...
```{r}
max_lf_plot <- max_life_year_allocation%>%
  ggplot(aes(x = age, y = SOFA, color = get_vent)) +
  geom_point() + labs(color = "") +
  scale_y_discrete(drop = FALSE)
...
combined_df <- 
  ...
  mutate(system = "Lottery",
         triage_cat = case_when(
              sofa_num < 7 ~ "Highest",
              sofa_num < 12 ~ "Intermediate",
              TRUE ~ "No ventilator")) %>%
  select(age, SOFA, get_vent, triage_cat, system) %>%
  rbind(sickest_first_allocation %>% 
  ...
  rbind(ny_allocation %>% 
    ...
    rbind(young_allocation %>% 
      ...
      rbind(max_icu_surv_allocation%>% 
          ...
          select(age, SOFA, get_vent, triage_cat, system)) %>%
        rbind(max_life_year_allocation %>% 
...
combined_plot <- combined_df %>%
  ggplot(aes(x = age, y = SOFA, color = get_vent)) +
  geom_point() + facet_wrap(~system) + 
  scale_y_discrete(breaks = c("0", "5", "10", "15", ">=20"))+
  labs(color = " ", x = "Age", shape = "NY triage category") + 
...
ggsave("Figure_1.pdf", combined_plot, height = 4, width= 7)
```
...
description <- c( "highest SOFA score first",
                  "random assignment",
                 "life years (100-age)",
                 "(SOFA<7, SOFA 8-11, SOFA>11) tiers",
                 "lowest SOFA score first",
                 "P(hospital survival)*(100-age)"
                 )

            lives <- c(lives_saved(sickest_first_allocation),
                       lives_saved(lottery_allocation),
                       young_lives,
                       ny_lives,
                       max_icu_lives,
                       max_lf_lives)
            
            life_years <- c(life_years_saved(sickest_first_allocation),
...
There were `r total_life_years` possible life-years to be saved
