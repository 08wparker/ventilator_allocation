...
simulate_ICU_pop <- function(df, N = tot_patients, sd_prob = 1.5){
  ...
  for (n_cats in age_cats) {
    ...
    #sd_prob <- sqrt(qnorm(subgroup$survival*(1-subgroup$survival)))
...
A ICU survival triage system of `r num_vents` ventilators would save `r lives_saved(max_icu_surv_allocation)` out of `r tot_patients` patients admitted to the ICU. Maximizing ICU survival saves `r comma(life_years_saved(max_icu_surv_allocation))` out of a total of possible `r comma(total_life_years)` (`r round(100*life_years_saved(max_icu_surv_allocation)/total_life_years)`%).
...
Prioritizing life-years for `r num_vents` ventilators would save `r lives_saved(max_life_year_allocation )` out of `r tot_patients` patients admitted to the ICU. Maximizing life-years gained saves `r comma(life_years_saved(max_life_year_allocation))` out of a total of possible `r comma(total_life_years)` (`r round(100*life_years_saved(max_life_year_allocation)/total_life_years)`%).


## Maximizing life-years vs. ICU survival

### The Tradeoff
Prioritizing life-years gained over ICU survival saves an additional `r comma(life_years_saved(max_life_year_allocation) - life_years_saved(max_icu_surv_allocation))` life-years for this `r tot_patients` patient sample, at a cost of `r lives_saved(max_icu_surv_allocation) - lives_saved(max_life_year_allocation)` more deaths in the ICU. 
